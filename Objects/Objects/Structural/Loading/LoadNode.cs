...
namespace Objects.Structural.Loading
{
  public class LoadNode : Load
  {
    ...
    /// <param name="value">The magnitude of the load, either a force or moment</param>
    /// <param name="name">A name or description to identify the load</param>/// 
    [SchemaInfo("Node Load (user-defined axis)", "Creates a Speckle node load (specifed using a user-defined axis)", "Structural", "Loading")]
    public LoadNode(LoadCase loadCase, List<Node> nodes, Axis loadAxis, LoadDirection direction, double value, string name = null)
    {
      this.name = name;
      this.loadCase = loadCase;
      this.nodes = nodes;
