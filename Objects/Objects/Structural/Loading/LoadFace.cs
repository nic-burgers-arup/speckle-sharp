...
namespace Objects.Structural.Loading
{
  public class LoadFace : Load
  {
    ...
    public LoadFace(LoadCase loadCase, List<Base> elements, FaceLoadType loadType, LoadDirection2D direction, LoadAxisType loadAxis = LoadAxisType.Global,
        ...
        bool isProjected = false, string name = null)
    {
      this.loadCase = loadCase;
      this.elements = elements;
      this.loadType = loadType;
      this.direction = direction;
      this.loadAxisType = loadAxis;
      this.values = values;
    ...
    /// <param name="isProjected">Whether the load is projected (ie. whether the distributed load is specified as the intensity applied to the projection of the element on the surface normal to the direction of the load, like snow in an inclined roof)</param>
    /// <param name="name">A name or description to identify the load</param>
    [SchemaInfo("Face Load (user-defined axis)", "Creates a Speckle structural face (2D elem/member) load (specified using a user-defined axis)", "Structural", "Loading")]
    public LoadFace(LoadCase loadCase, List<Base> elements, FaceLoadType loadType, LoadDirection2D direction, Axis loadAxis,
        [SchemaParamInfo("A list that represents load magnitude (number of values varies based on load type - Uniform: 1, Variable: 4 (corner nodes), Point: 1)")] List<double> values = null,
        [SchemaParamInfo("A list that represents load locations (number of values varies based on load type - Uniform: null, Variable: null, Point: 2)")] List<double> positions = null,
        bool isProjected = false, string name = null)
    {
