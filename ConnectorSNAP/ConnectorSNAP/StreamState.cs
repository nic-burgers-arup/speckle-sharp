...
namespace ConnectorSNAP
{
  ...
  public class StreamState
  {
    ...
    public async Task<bool> RefreshStream(IProgress<MessageEventArgs> loggingProgress)
    {
      ...
      {
        loggingProgress.Report(new MessageEventArgs(Speckle.SNAP.API.MessageIntent.Display, Speckle.SNAP.API.MessageLevel.Error, "Unable to fetch stream"));
        loggingProgress.Report(new MessageEventArgs(Speckle.SNAP.API.MessageIntent.TechnicalLog, Speckle.SNAP.API.MessageLevel.Error, ex, "Unable to fetch stream"));
        return false;
      }
